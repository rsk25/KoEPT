...
from page.util_kor import ExpectedTimeToFinishCalculator
...
class RayBatchActor(object):
    def train(self, kwargs):
        trainer = Trainer(kwargs['name'] + '/' + kwargs['runname'],
                          ...
                          kwargs['data_path'][0], kwargs['data_path'][-1], dev=None,
        ...
        trainer.close()
        return [kwargs['name'], kwargs['runname'], time_delta, test_result]
...
if __name__ == '__main__':
    ...
    for num_gpu, experiments in experiment_map.items():
        ...
        pool = ActorPool([RayBatchActor.options(num_cpus=8, num_gpus=num_gpu).remote()
        ...
        for name, results in groupby(sorted(exp_result, key=lambda t: t[0]), key=lambda t: t[0]):
            ...
            for name, runname, timedelta, testresult in results:
            ...
            if len(results) > 1:
                _, _, time_delta, test_results = zip(*results)
