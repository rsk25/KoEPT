...
from transformers.tokenization_kobert import KoBertTokenizer
...
if __name__ == '__main__':
    ...
    for setname in tqdm(args.set):
        ...
        for qid, text, expr, ans in tqdm(dataset):
            ...
            for field in field_to_test:
                if field is None:
                    e = [[FORMAT_NUM % int(tok[2:]) if tok.startswith(NUM_PREFIX) 
                          else tok for typ, eqn in expr if typ != PREP_KEY_ANS for tok in eqn.split(' ')]]
                else:
