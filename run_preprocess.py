...
def preprocess_each():
    ...
    from page.preprocess.mawps import MAWPSPreprocessor, CC_ILPreprocessor
    ...
    *********************************
    ...
    #             json.dump(all_problems, fp,ensure_ascii=False)
    ...
    # Preprocess CC or IL
    if not (Path(args.output, 'cc_fold4_test.jsonl').exists() and Path(args.output, 'il_fold4_test.jsonl').exists()):
        print("\033[1;44;38mBegin to process CC or IL.\033[0m")
        preproc = CC_ILPreprocessor()

        # Add CC or IL path
        path = Path(args.input, args.ccil)
        folds = []
        for i in range(5):
            with Path(path.parent, 'fold%s.txt' % i).open(encoding='UTF-8') as fp:
                indices = [int(line.strip()) for line in fp.readlines() if line.strip()]
        ...
        if args.ccil.find('CC') != -1:
            preproc.add_fold_information(path, Path(args.output, 'CC'), 'CC', key_field='iIndex', folds=folds)
        else:
            preproc.add_fold_information(path, Path(args.output, 'IL'), 'IL', key_field='iIndex', folds=folds)
        preproc.run()
    else:
        print("\033[1;32mCC or IL already exists\033[0m")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ccil',type=str, help='Path of CC/IL files in input directory',
                        default='input/CC/CC_kor.json')
